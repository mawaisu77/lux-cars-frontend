  ...
  return (
    <div className="flex gap-[0.5vw] flex-col   lg:flex-row justify-between">
      <div className="flex flex-1 px-[0.5vw] gap-2 lg:gap-[0.5vw] items-center lg:w-[16vw] rounded-[0.5vw] bg-white">
        <div className="flex justify-center items-center rounded-lg lg:rounded-[0.5vw] p-1 lg:p-[0.5vw] bg-[#CA0000]">
          <PiUsersFill color="white" className="text-[18px] lg:text-[1vw]" />
        </div>
        
        <div data-tooltip-id={`seller-tooltip`} className="text-left p-2 md:p-[0.3vw]">
          <p className="text-[14px] lg:text-[0.8vw] text-[#6a6978] font-semibold tracking-wider">Owned by <span style={{color:  documentOldType?.color }}> ( { documentCategory }) </span> </p>
      ...
      <div className="flex flex-1 px-[0.5vw] gap-2 lg:gap-[0.5vw] items-center lg:w-[16vw] rounded-[0.5vw] bg-white">
      <div className="flex justify-center items-center rounded-lg lg:rounded-[0.5vw] p-1 lg:p-[0.5vw]  bg-[#CA0000]">
          <CgFileDocument color="white" className="text-[18px] lg:text-[1vw]" />
        </div>
        <div className="text-left p-[0.5vw] md:p-[0.3vw] " data-tooltip-id={`document-${documentOldType?.label}`}>
          <p className="text-[14px] lg:text-[0.8vw] text-[#6a6978] font-semibold tracking-wider">Sale Documents</p>
          <p 
            className={`text-[14px] lg:text-[0.8vw] font-urbanist font-semibold  text-[${documentOldType?.color}]`} style={{ color: documentOldType?.color || '', padding: documentOldType ? '0.2vw' : '0', borderRadius: documentOldType ? '0.2vw' : '0' }}>
